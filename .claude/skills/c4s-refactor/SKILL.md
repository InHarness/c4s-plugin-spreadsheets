...
carries its identity (`--project 'c4s-plugin-spreadsheets' --workspace 'default'`), so it works from any cwd. Do NOT `cd`
...
```sh
c4s catalog --project 'c4s-plugin-spreadsheets' --workspace 'default'                                   # entity types + schemas
c4s list-tags --project 'c4s-plugin-spreadsheets' --workspace 'default'                                 # tags + counts
c4s list-slugs --type endpoint --project 'c4s-plugin-spreadsheets' --workspace 'default'                # slugs for a type
c4s single_element --type endpoint --slug <x> --project 'c4s-plugin-spreadsheets' --workspace 'default'
c4s resolve modules/<module>.md --project 'c4s-plugin-spreadsheets' --workspace 'default'               # expand a page's tags inline
```
...
not execute." --project 'c4s-plugin-spreadsheets' --workspace 'default'
```
...
the implementer must change and why>" --ct brief --source analysis --project 'c4s-plugin-spreadsheets' --workspace 'default'
```
...
- **The identity is baked in — never `cd`.** `--project 'c4s-plugin-spreadsheets' --workspace 'default'` is injected into every
