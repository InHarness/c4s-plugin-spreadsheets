...
command below carries its identity (`--project 'c4s-plugin-spreadsheets' --workspace 'default'`), so it works from any cwd,
...
XML tag names 1:1 — append `--project 'c4s-plugin-spreadsheets' --workspace 'default'` to every command below.

| XML tag | CLI equivalent |
|---------|----------------|
| `<inline_mention type="endpoint" slug="X"/>` | `c4s inline_mention --type endpoint --slug X --project 'c4s-plugin-spreadsheets' --workspace 'default'` |
| `<single_element type="dto" slug="X"/>` | `c4s single_element --type dto --slug X --project 'c4s-plugin-spreadsheets' --workspace 'default'` |
| `<element_list type="endpoint" slugs="a,b,c"/>` | `c4s element_list --type endpoint --slugs a,b,c --project 'c4s-plugin-spreadsheets' --workspace 'default'` |
| `<tagged_list type="dto" tags="auth" filter="and"/>` | `c4s tagged_list --type dto --tags auth --filter and --project 'c4s-plugin-spreadsheets' --workspace 'default'` |
| `<tagged_list_mixed tags="public"/>` | `c4s tagged_list_mixed --tags public --project 'c4s-plugin-spreadsheets' --workspace 'default'` |

## Expanding a whole page

```sh
c4s resolve some-page.md --project 'c4s-plugin-spreadsheets' --workspace 'default'                # writes markdown with tags expanded inline
c4s resolve some-page.md --format json --project 'c4s-plugin-spreadsheets' --workspace 'default'   # writes { content, resolved: [...] }
```

## Discovery

- `c4s catalog --project 'c4s-plugin-spreadsheets' --workspace 'default'` — active entity types with counts + version + description + roleNoun + mcpToolsLine per type (smoke test).
- `c4s describe --type <t> [--view <v>] --project 'c4s-plugin-spreadsheets' --workspace 'default'` — JSON Schema per view for one type (on-demand).
- `c4s list-tags --project 'c4s-plugin-spreadsheets' --workspace 'default'` — all tags with per-type counts.
- `c4s list-slugs --type endpoint --project 'c4s-plugin-spreadsheets' --workspace 'default'` — all slugs for a given type.
...
```sh
c4s ask "<question>" --project 'c4s-plugin-spreadsheets' --workspace 'default'
```
...
`AMBIGUOUS_PROJECT`, this skill's baked-in `--project 'c4s-plugin-spreadsheets' --workspace 'default'` identity no longer
